= Using Containerized AAP2

Whilst we have lightly touched on the new Postinstall capability built on top of the foo collection in this Bonus lab we will actually setup a simple AAP2 job_template and assocaited resources. We will do this manually as it allows us to experiewnce the Containerized AAP2 UI (User Interface) and realize that it is identical to the Ansible Automation Controller UI from an instance/VM based install and indeed an OpenShift install.
...
. Now navigate the the *Templates* screen and open the *rh1-demo-job-template*
+
...
Congratulations you have now completed the lab *but* should you have time to spare then areas to explore include:
...
Enjoy the rest of your Red Hat One and thanks for your time today
